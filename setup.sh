...
docker-compose exec wordpress apt install -y less git subversion
...
docker-compose exec --user www-data wordpress php -d memory_limit=256M /usr/local/bin/wp package install git@github.com:Sidsector9/ee-command.git
